...
const obtenerProyecto = async (req, res) => {
    ...
    if(proyecto.creador.toString() !== req.usuario._id.toString() && !proyecto.colaboradores.some(
        ...
        const error = new Error('PROYECTO NO TE PERTENECE')
...
const buscarColaborador = async (req, res) => {
    ...
    if(usuario._id.toString() === proyecto.creador.toString()){
        const error = new Error('Ya eres administrador por lo tanto colaborador de este proyecto');
        return res.status(404).json({msg: error.message})
    }
...
const agregarColaborador = async (req, res) => {
    ...
    if(proyecto.colaboradores.includes(usuario._id)){
        const error = new Error('Este usuario ya es colaborador de este proyecto');
        return res.status(404).json({msg: error.message})
    }
